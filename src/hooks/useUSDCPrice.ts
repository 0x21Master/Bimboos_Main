...
import { useGetTokenPrice } from 'state/tokens/hooks'
...
const STABLECOIN_AMOUNT_OUT: { [chainId: number]: CurrencyAmount<Token> } = {
  ...
  // [SupportedChainId.KOVAN]: CurrencyAmount.fromRawAmount(DAI_OPTIMISM, 10_000e18),
}
...
export function useUSDCPriceWithCache(
  chainId: string | undefined,
  currency?: Token | undefined
): Price<Currency, Token> | undefined {
  const tokenPrice = useGetTokenPrice(chainId, currency)
  if (!tokenPrice || !currency) {
    return undefined
  }
  const price = new Price(currency, USDC, tokenPrice.denominator, tokenPrice.numerator)
  return price
}
